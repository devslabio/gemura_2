...
export default function Modal({ open, onClose, title, children, footer, maxWidth = 'max-w-lg' }: ModalProps) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-5 min-h-screen min-h-[100dvh]" role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <div className="absolute inset-0 bg-black/50" style={{ minHeight: '100dvh' }} onClick={onClose} aria-hidden="true" />
      <div className={`relative w-full ${maxWidth} max-h-[90vh] flex flex-col bg-white rounded-lg border border-gray-200 shadow-xl`} onClick={(e) => e.stopPropagation()}>
        ...
        {footer != null && <div className="shrink-0 border-t border-gray-200 px-6 py-4 flex items-center justify-end gap-3">{footer}</div>}
      </div>
    </div>
  );
