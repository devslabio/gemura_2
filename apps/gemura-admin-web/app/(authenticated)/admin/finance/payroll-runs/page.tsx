...
export default function PayrollRunsReportPage() {
  ...
  const periodLabel = period
    ? `UTC window: ${new Date(period.start).toLocaleString()} → ${new Date(period.end).toLocaleString()} · completed runs only`
    : undefined;
  ...
  if (loading && rows.length === 0) {
    return (
      <PlatformDrilldownShell title="Payroll runs" backHref={backHref}>
  ...
  return (
    <PlatformDrilldownShell title="Payroll runs (period)" periodLabel={periodLabel} backHref={backHref}>
      {error ? (
